"""Headless Fast-FoundationStereo model adapter for the HTTP runtime.

Resolution order for weights:
  1. $FFS_WEIGHTS_DIR
  2. <this runtime dir>/weights  (default, populated by download.sh)
...
def _weights_dir() -> Path:
    ...
    if not candidate.exists():
        raise RuntimeError(
            f"FFS weights not found at {candidate}. Run `robot-tools download` (or `bash download.sh`)."
        )
    return candidate
...
def _resolve_model(model_id: str) -> Path:
    ...
    if pths:
        return pths[0]
    raise FileNotFoundError(f"FFS checkpoint {model_id!r} not found. Tried {preferred} and any *.pth under {base}.")
...
class FastFSBackend:
    ...
    def infer(
        ...
        # msgpack decoding may produce read-only array views. PyTorch warns
        # that tensors backed by those arrays have undefined write behavior,
        # so copy only when the incoming storage is not writable.
        if not left_u8.flags.writeable:
            left_u8 = left_u8.copy()
        if not right_u8.flags.writeable:
            right_u8 = right_u8.copy()
        ...
        with torch.amp.autocast("cuda", enabled=True, dtype=AMP_DTYPE):
            disp = self.model.forward(t0, t1, iters=vi, test_mode=True, optimize_build_volume="pytorch1")
