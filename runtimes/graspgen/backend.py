...
# The upstream is a pinned source submodule, not an installed distribution.
# Keeping it on the import path lets the real lock own every dependency rather
# than inheriting GraspGen's training/UI-oriented package metadata.
...
def _models_dir() -> Path:
    ...
    if not candidate.exists():
        raise RuntimeError(
            ...
            "Run `robot-tools download` (or `bash download.sh`)."
        )
    return candidate
...
def _resolve_gripper_config(gripper_config: str) -> str:
    ...
    raise FileNotFoundError(f"Gripper config {gripper_config!r} not found in {cfg_dir}. Available: {available}")
...
class GraspGenBackend:
    ...
    def __init__(
        ...
        if source_name != target_name:
            self._z_offset = float(get_gripper_depth(source_name)) - float(get_gripper_depth(target_name))
        else:
            self._z_offset = 0.0
    ...
    def generate_grasps(self, object_pc: np.ndarray) -> dict:
        ...
        if len(grasps) == 0:
            return {"poses": np.empty((0, 4, 4), dtype=np.float32), "confidences": np.empty((0,), dtype=np.float32)}
